...
class AppTest {
    ...
    // BEGIN
    @Test
    void testRegistrationPage() throws IOException {
        CloseableHttpClient client = HttpClients.createDefault();
        HttpGet getRequest1 = new HttpGet(baseUrl + "/users/new");

        CloseableHttpResponse response = client.execute(getRequest1);
        assertThat(response.getCode()).isEqualTo(200);
    }
    // END
